...
export function ConnectionError({ message }: { message: string | null }) {
  ...
  return (
    <div className="flex flex-1 items-center justify-center p-4">
      <div className="w-full max-w-md border border-danger bg-danger-dim/10">
        <div className="border-b border-danger px-4 py-2">
          <h2 className="text-[11px] font-semibold tracking-[0.22em] text-danger">
            {missingEnv ? "RPC NOT CONFIGURED" : "CAN'T REACH SOLANA DEVNET"}
          </h2>
        </div>
        <div className="p-4">
          <p className="text-[12px] leading-relaxed text-bone-dim">{msg}</p>
          <ul className="mt-3 space-y-1 text-[10px] leading-relaxed text-bone-faint">
            {missingEnv ? (
              <>
